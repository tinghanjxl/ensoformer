...
class ensoformer(nn.Module):
    def __init__(self, mypara):
        ...
        enc_layer = miniEncoder(  # 包含多头时空注意力和全连接层
    ...
    def encode(self, predictor, in_mask):
        ...
        predictor = self.predictor_emb(predictor)   # 线性、位置、时间嵌入
...
class multi_enc_layer(nn.Module):
    def __init__(self, enc_layer, num_layers):
        ...
        self.skip_linear = nn.Linear(2 * mypara.d_size, mypara.d_size)
    ...
    def forward(self, x, mask=None):
        ...
        for layer in self.layers:
            ...
            if count <= len(self.layers) // 2:
                x = layer(x, mask)
                residuals.append(x)
            elif count == len(self.layers) // 2 + 1:
                x = layer(x, mask)
            else:
                x = self.skip_linear(torch.cat([x, residuals.pop()], dim=-1))
                x = layer(x, mask)
...
if __name__ == '__main__':
    input_var = numpy.ones((1,12,9,51,120))
    input_var = torch.from_numpy(input_var)
    var_true  = numpy.ones((1,20,9,51,120))
    var_true = torch.from_numpy(var_true)
    mymodel = ensoformer(mypara).to(mypara.device)
    var_pred = mymodel(
        input_var.float().to('cuda'),
        var_true.float().to('cuda'),
        train=True,
        sv_ratio=0,
    )
