...
class TFtrainer:
    def __init__(self, mypara):
        ...
        self.adam = torch.optim.AdamW(self.mymodel.parameters(), lr=mypara.TFlr)
    ...
    def transfer_learning(self, dataset_train, dataset_eval, adr_model):
        ...
        chk_path = self.mypara.model_savepath + "ENSO-Former-TF.pkl"
        self.mymodel.load_state_dict(torch.load(adr_model))
        print("\n-->load ENSO model: {0}".format(adr_model))
        count = 0
        (
            _,
            _,
